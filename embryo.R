...
datasets <- list(
  list(path = "data/48hpf_et31", age = "48hpf"),
  list(path = "data/48hpf_et33", age = "48hpf"),
  list(path = "data/72hpf_et31", age = "72hpf"),
  list(path = "data/72hpf_et33", age = "72hpf")
)

# Process all datasets
objs_list <- map(datasets, ~process_dataset(.x$path, .x$age))

saveRDS("output/embryo_raw_data.rds")
...
saveRDS(
  integrated,
  file = "output/embryo_integrated.rds"
)
...
writexl::write_xlsx(
  markers,
  path = "output/embryo_markers.xlsx"
)
...
write.table(
  signature_matrix,
  file = "output/embryo_signature_matrix.txt",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  signature_matrix,
  file = "output/embryo_signature_matrix_zfin.txt",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)